import type { Request, Response } from 'express';
import { db } from '../../../db/client.js';
import { investors } from '../../../db/schema.js';
import { eq } from 'drizzle-orm';
import { sendEmail, getInvestorVerificationEmail } from '../../../lib/email.js';

export default async function handler(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const { verificationStatus } = req.body;

    if (!id) {
      return res.status(400).json({
        success: false,
        error: 'Investor ID is required'
      });
    }

    if (!verificationStatus) {
      return res.status(400).json({
        success: false,
        error: 'Verification status is required'
      });
    }

    // Validate status
    const validStatuses = ['pending', 'verified', 'rejected'];
    if (!validStatuses.includes(verificationStatus)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid verification status. Must be: pending, verified, or rejected'
      });
    }

    // Fetch investor before update to get email and name
    const existing = await db
      .select()
      .from(investors)
      .where(eq(investors.id, parseInt(id)))
      .limit(1);

    if (existing.length === 0) {
      return res.status(404).json({
        success: false,
        error: 'Investor not found'
      });
    }

    const investor = existing[0];

    // Update the investor
    await db
      .update(investors)
      .set({ 
        verificationStatus,
        updatedAt: new Date()
      })
      .where(eq(investors.id, parseInt(id)));

    // Send email notification if status is verified or rejected
    if (verificationStatus === 'verified' || verificationStatus === 'rejected') {
      const { subject, html } = getInvestorVerificationEmail(
        investor.fullName,
        verificationStatus as 'verified' | 'rejected'
      );
      
      await sendEmail({
        to: investor.email,
        subject,
        html,
      });
    }

    // Fetch the updated investor
    const updated = await db
      .select()
      .from(investors)
      .where(eq(investors.id, parseInt(id)))
      .limit(1);

    res.json({
      success: true,
      data: updated[0],
      message: 'Investor verification status updated and notification sent'
    });
  } catch (error) {
    console.error('Error updating investor:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to update investor',
      message: String(error)
    });
  }
}
