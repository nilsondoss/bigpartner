/**
 * Add Layout Property Listing
 * Adds the 10 acre layout land property to the database
 */

import { db } from './client.js';
import { properties } from './schema.js';

async function addLayoutProperty() {
  try {
    console.log('🌱 Adding layout property...');
    
    const property = {
      // Basic Information
      title: 'Land for Layout – 15 km from Chennai',
      slug: 'land-for-layout-15-km-chennai',
...
• Any other relevant documents as per seller`,
    ...
    console.log(`Property ID: ${result[0].insertId}`);
    console.log('Title: Land for Layout – 15 km from Chennai');
    console.log('Type: Farmland');
    console.log('Area: 10 acres (435,600 sq.ft)');
    console.log('Location: Within 15 km from Chennai');
